...
export const CancellationScreen = () => {

  const [cancellation, setCenclalation] = useState("")
    ...
    const fetchDeliveryType = async (e) => {
      ...
      try {
        ...
        if (payment_types?.status === 200) {
          // alert("AA GYA")
          console.log(ticketAdd.cancellation_policy.content, "---------------TERM AND CONDITION---------------");
          setCenclalation(ticketAdd.cancellation_policy.content);
        }
        setIsLoading(false)
      }
      catch (error) {
  ...
  return (
    <div className='mt-16 py-10'>
      <MainHeading name="Cancellation Policy" />
      ...
      {isLoading ? (
                ...
                <div dangerouslySetInnerHTML={{ __html: cancellation }} className='bg-gray-100 font-subheading max-w-[1000px] m-auto p-6 rounded-lg' />
            )}
      {/* <div dangerouslySetInnerHTML={{ __html: cancellation }} className='bg-gray-100 font-subheading max-w-[1000px] m-auto p-6 rounded-lg' /> */}
    </div>
  )
